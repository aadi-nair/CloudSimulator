...
import org.cloudbus.cloudsim.utilizationmodels.{UtilizationModelFull, UtilizationModel}
...
class CloudletSchedulerSimulation(val configName: String, val simulationName: String, val cloudletScheduler: CloudletScheduler):
  val logger: Logger = CreateLogger(classOf[CloudletSchedulerSimulation])
  ...
  val datacenter: Datacenter = dcLoader.loadDatacenter(simulation, VmAllocationPolicySimple(), VmSchedulerTimeShared(), true)


  val broker = DatacenterBrokerSimple(simulation)

  broker.submitVmList(dcLoader.vmList(cloudletScheduler).asJava)
...
//  dcLoader.calculateCost(broker)

//  broker.submitVmList(dcLoader.vmList(cloudletScheduler))
